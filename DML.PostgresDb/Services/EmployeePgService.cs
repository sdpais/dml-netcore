...
public class EmployeePgService : IEmployeePgService
{
    private readonly IDbServiceAsync _dbService;

    public EmployeePgService(IDbServiceAsync  dbService)
    {
    ...
    {
        var employeeList = await _dbService.GetMany<Employee>("SELECT * FROM public.employees", new { });
        return employeeList;
    }
    ...
    {
        var employee = await _dbService.GetFirst<Employee>("SELECT * FROM public.employees where id=@id", new {id});
        return employee;
    }
    public async Task<Employee> GetEmployee(string email)
    {
        var employee = await _dbService.GetFirst<Employee>("SELECT * FROM public.employees where email=@email", new { email});
        return employee;
    }
